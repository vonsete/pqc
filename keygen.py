...
from pem_utils import encode_pem
...
guardar_pem(KEY_DIR / "mlkem_secret.pem",  encode_pem("ML-KEM-1024 PRIVATE KEY", mlkem_priv), es_privada=True)
print(f"[ML-KEM-1024]  pública: {len(mlkem_pub)} B | privada: {len(mlkem_priv)} B")
...
guardar_pem(KEY_DIR / "mldsa_secret.pem",  encode_pem("ML-DSA-87 PRIVATE KEY", mldsa_priv), es_privada=True)
print(f"[ML-DSA-87]    pública: {len(mldsa_pub)} B | privada: {len(mldsa_priv)} B")
